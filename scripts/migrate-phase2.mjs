...
async function migrate() {
  ...
  try {
    ...
    for (const t of tables) {
      const policyName = `${t}_authenticated`
      try { await sql.unsafe(`DROP POLICY IF EXISTS "${policyName}" ON ${t}`) } catch {}
      await sql.unsafe(`CREATE POLICY "${policyName}" ON ${t} FOR ALL TO authenticated USING (true) WITH CHECK (true)`)
    }

    // Reference tables - read only
    try { await sql`DROP POLICY IF EXISTS "dn_diametres_read" ON dn_diametres` } catch {}
    await sql`CREATE POLICY "dn_diametres_read" ON dn_diametres FOR SELECT TO authenticated USING (true)`
    try { await sql`DROP POLICY IF EXISTS "localisations_read" ON localisations_type` } catch {}
